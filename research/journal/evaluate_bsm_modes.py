...
class ErrorControlLUT:
    ...
    def get_optimal_params(self, current_pdr: float, current_burst: float) -> Tuple[int, int]:
        # 💡 unpack 에러 방지를 위해 return (0, 0)으로 수정
...
class ChannelMetrics:
    ...
    def get_metrics(self, mode: str, trace_idx: int, window_bits: int = 100) -> Tuple[float, float]:
        ...
        # 💡 [정답] NAIVE 파일이든 ARGUS 파일이든 이미 .shift(1)이 적용된 Pred_PDR_cons 칼럼만 읽는다!
...
def main():
    ...
    print("\n[ Phase 1: Running Baselines, NAIVE, and Theoretical Upper Bound ]")
    ...
    for i, bar in enumerate(bars):
        ...
        ax1.text(bar.get_x() + bar.get_width()/2, yval - 0.5, # -1.0 만큼 아래로 내려 막대 안으로
                 f"{yval:.2f}%", ha='center', va='top', fontweight='bold', fontsize=10, color=text_color)
    ...
    ax1.axvspan(2.5, 8.5, facecolor='#17becf', alpha=0.1, zorder=0)
